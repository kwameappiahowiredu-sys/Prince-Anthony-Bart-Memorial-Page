...
(function () {
  ...
  /* Remembers which posts THIS browser created, so only the person who
     posted something sees a Delete button on it. This is a convenience,
     not a security boundary — see SETUP.md. */
  function ownedKey(kind) { return "memorial_owned_" + kind; }
  function getOwned(kind) {
    try { return JSON.parse(localStorage.getItem(ownedKey(kind)) || "[]"); }
    catch (e) { return []; }
  }
  function addOwned(kind, id) {
    try {
      var list = getOwned(kind);
      list.push(id);
      localStorage.setItem(ownedKey(kind), JSON.stringify(list));
    } catch (e) {}
  }
  function removeOwned(kind, id) {
    try {
      var list = getOwned(kind).filter(function (x) { return x !== id; });
      localStorage.setItem(ownedKey(kind), JSON.stringify(list));
    } catch (e) {}
  }
  ...
  db.collection("tributes").orderBy("createdAt", "desc").onSnapshot(function (snap) {
    ...
    var ownedTributes = getOwned("tributes");
    snap.forEach(function (doc) {
      ...
      if (ownedTributes.indexOf(doc.id) !== -1) {
        el.appendChild(makeDeleteButton(function () {
          db.collection("tributes").doc(doc.id).delete()
            .then(function () { removeOwned("tributes", doc.id); })
            .catch(function (err) { alert("Sorry, that couldn't be deleted. Please try again."); console.error(err); });
        }));
      }
      wall.appendChild(el);
    });
  ...
  db.collection("photos").orderBy("createdAt", "desc").onSnapshot(function (snap) {
    ...
    var ownedPhotos = getOwned("photos");
    snap.forEach(function (doc) {
      var p = doc.data();
      ...
      if (ownedPhotos.indexOf(doc.id) !== -1) {
        fig.appendChild(makeDeleteButton(function () {
          db.collection("photos").doc(doc.id).delete()
            .then(function () { removeOwned("photos", doc.id); })
            .catch(function (err) { alert("Sorry, that couldn't be deleted. Please try again."); console.error(err); });
        }));
      }
  ...
  db.collection("videos").orderBy("createdAt", "desc").onSnapshot(function (snap) {
    ...
    var ownedVideos = getOwned("videos");
    snap.forEach(function (doc) {
      var v = doc.data();
      var fig = document.createElement("figure"); fig.className = "video-tile";
      fig.dataset.src = v.url;
      var vid = document.createElement("video");
      vid.src = v.url; vid.preload = "metadata"; vid.muted = true;
      vid.setAttribute("playsinline", "");
      fig.appendChild(vid);
      var play = document.createElement("div"); play.className = "play-overlay";
      play.innerHTML = '<span><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span>';
      fig.appendChild(play);
      var cap = document.createElement("figcaption");
      var who = v.name ? v.name : "Anonymous";
      cap.textContent = (v.caption ? v.caption + " — " : "") + who + " · " + fmt(v.createdAt);
      fig.appendChild(cap);
      if (ownedVideos.indexOf(doc.id) !== -1) {
        fig.appendChild(makeDeleteButton(function () {
          db.collection("videos").doc(doc.id).delete()
            .then(function () {
              removeOwned("videos", doc.id);
              if (v.path) storage.ref().child(v.path).delete().catch(function () {});
            })
            .catch(function (err) { alert("Sorry, that couldn't be deleted. Please try again."); console.error(err); });
        }));
      }
